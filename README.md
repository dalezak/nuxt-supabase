...
A lean, OSS Nuxt 4 layer for Supabase apps. Provides core data-model primitives — `SupaModel` / `SupaModels` / `RestModel` / `RestModels` base classes wrapping common Supabase query patterns, plus auth/profile (`users`), local storage (`useStorage`), and the `createSupaStore` Pinia helper. Everything else (friends, badges, plans, notifications, principles, courses, streaks, likes) lives in opt-in sub-layers so apps include only what they need.
...
- **`nuxt-plans`** *(private)* — billing: ships the `subscription_status` / `subscription_expires_at` / `subscription_platform` columns as a migration extending `nuxt-supabase`'s `users` table + generic `usePlan()` composable (reads `users.subscription_status` + `useAppConfig().plans`) + `revenuecat` Edge Function webhook handler. Renamed from `nuxt-subscriptions` to avoid name collision with the push `subscriptions` table in `nuxt-notifications`.
